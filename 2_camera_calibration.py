...
images = glob.glob(f'./data/calibration/calib_images/*.png')
...
cv_file = cv2.FileStorage('./data/calibration/calib_cam.xml', cv2.FILE_STORAGE_WRITE)
...
cv2.imwrite('./utils/orig.png', img)

undst = cv2.undistort(img, cameraMatrix, dist, None, newcameramtx)
cv2.imwrite('./utils/undst.png', undst)
